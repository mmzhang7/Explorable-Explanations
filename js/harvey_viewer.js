...
// D3 vars
let canvas, ctx, harveyProjection, colorScale, timestampLabel;
...
function createViewerUI(initialIndex = 0) {
  ...
  harveyProjection = d3.geoMercator()
    .scale(1) 
    .translate([width / 2, height / 2]);

  // fixed projection scale/center once
  harveyProjection.fitExtent([[0, 0], [width, height]], GLOBAL_BOUNDS_GEOJSON);
...
function updateViewer(fileIndex) {
  ...
  for (const d of data) {
    const [x, y] = harveyProjection([d.lon, d.lat]);
